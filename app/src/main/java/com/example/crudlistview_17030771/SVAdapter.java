package com.example.crudlistview_17030771;
...
public class SVAdapter extends RecyclerView.Adapter<SVAdapter.MyViewHolder> {

    private Context context;
    private ArrayList<SV> SVArrayList;
    private String url="https://5fd1f08fb485ea0016eef074.mockapi.io/SV/";

    public SVAdapter(Context context, ArrayList<SV> SVArrayList) {
        this.context = context;
        this.SVArrayList = SVArrayList;
    }
    ...
    @NonNull
    @Override
    public SVAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view;

        LayoutInflater layoutInflater= LayoutInflater.from(context);
        view=layoutInflater.inflate(R.layout.svlist_item, parent, false);
        return new MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull SVAdapter.MyViewHolder holder, int position) {
        holder.id.setText("#"+ String.valueOf(position+1));
        holder.name.setText(SVArrayList.get(position).getName());
        holder.edit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                int id= SVArrayList.get(position).getId();
                String value= SVArrayList.get(position).getName();
                editSong(id, value);
            }
        });
